...
@pytest.fixture
def ind_cls():
    creator.create_type(MUT_FIT, Fitness, weights=(-1.0,))
    creator.create_type(MUT_IND, gp.PrimitiveTree, fitness=creator.__dict__[MUT_FIT])
...
def test_mut_shrink_shrinks_untyped_trees(ind_cls, seed):
    ...
    pset = _untyped_pset()
    tools.rng.seed(seed)
    tree = ind_cls(gp.gen_full(pset, min_depth=2, max_depth=3))
    before = len(tree)
...
def test_mut_shrink_leaves_tiny_trees_alone(ind_cls):
    pset = _untyped_pset()
    tree = ind_cls(gp.PrimitiveTree.from_string("ARG0", pset))
...
def test_mut_shrink_substitutes_argument_of_matching_type(ind_cls):
    ...
    tools.rng.seed(5)
    tree = ind_cls(gp.PrimitiveTree.from_string("add(ARG0, add(ARG0, ARG0))", pset))
...
def test_mut_uniform_replaces_a_subtree(ind_cls):
    pset = _untyped_pset()
    tools.rng.seed(21)
    tree = ind_cls(gp.gen_full(pset, min_depth=2, max_depth=2))
    before = str(tree)

    (mutant,) = gp.mut_uniform(tree, lambda prim_set, ret_type: gp.gen_full(prim_set, 1, 1), pset)
...
def test_mut_insert_grows_or_keeps_the_tree(ind_cls):
    pset = _untyped_pset()
    tools.rng.seed(23)
    tree = ind_cls(gp.gen_full(pset, min_depth=1, max_depth=2))
    before = len(tree)
...
def test_mut_ephemeral_modes(ind_cls):
    ...
    (one,) = gp.mut_ephemeral(tree, mode="one")
    (all_,) = gp.mut_ephemeral(one, mode="all")
    empty = ind_cls(gp.PrimitiveTree.from_string("ARG0", pset))
    (unchanged,) = gp.mut_ephemeral(empty, mode="all")
