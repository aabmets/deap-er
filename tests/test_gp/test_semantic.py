...
def test_semantic_crossover():
    ...
    ind1 = gp.gen_grow(pset, 1, 3)
    ind2 = gp.gen_grow(pset, 1, 3)
    new_ind1, new_ind2 = gp.cx_semantic(ind1, ind2, pset, max_depth=2)
...
def test_semantic_crossover_second_child_uses_original_first_parent():
    ...
    ind1 = gp.gen_grow(pset, 1, 2)
    ind2 = gp.gen_grow(pset, 1, 2)
...
def test_semantic_mutation():
    ...
    pset.add_primitive(operator.add, 2)
    pset.add_primitive(operator.mul, 2)
    individual = gp.gen_grow(pset, 1, 3)
    mutated = gp.mut_semantic(individual, pset, max_depth=2)
