...
from deap_er.private.operators.sel_age_moea_2_anchors import merge_worst, normalize_front
...
def test_memory_updates_on_exact_front_fill():
    fit_name = "AGE2_GEO_MEM_FIT"
    ind_name = "AGE2_GEO_MEM_IND"
...
def test_best_point_merges_without_worst_point():
    fit_name = "AGE2_GEO_BEST_FIT"
    ind_name = "AGE2_GEO_BEST_IND"
    creator.create_type(fit_name, Fitness, weights=(-1.0, -1.0))
    creator.create_type(ind_name, list, fitness=creator.__dict__[fit_name])
    try:
        pop = []
...
def test_worst_point_ignores_dominated_in_pool():
    fit_name = "AGE2_GEO_POOL_FIT"
    ind_name = "AGE2_GEO_POOL_IND"
    creator.create_type(fit_name, Fitness, weights=(-1.0, -1.0))
    creator.create_type(ind_name, list, fitness=creator.__dict__[fit_name])
    try:
        parents = []
...
def test_age2_oversize_geometry_uses_first_front_only():
    fit_name = "AGE2_GEO_OVERSIZE_FIT"
    ind_name = "AGE2_GEO_OVERSIZE_IND"
    creator.create_type(fit_name, Fitness, weights=(-1.0, -1.0))
    creator.create_type(ind_name, list, fitness=creator.__dict__[fit_name])
    try:
        front0 = []
