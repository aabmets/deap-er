...
from .case_exams import CaseSolved, ExamLike, bound_case_exams, elite_solve_matrix, exam_difficulty
...
def guard_case_exams(
    ...
    *,
    ...
    an informed resample of ``min_cases``. A fully solved exam unions
    ``held_out`` and, if still collapsed, bumps the subset size through
    ``sample_informed_cases``. Chronological splits stay on the caller.
...
def _repair_case_exam(
    ...
    if not cases:
        ...
        if not cases:
            ...
            return
    if exam_difficulty(solve, exam.as_cases(n_cases), "unsolved") > 0:
        return
...
def _union_case_exams(left: CaseExam, right: CaseExam, n_cases: int) -> CaseExam:
    return CaseExam(mask=left.as_mask(n_cases) | right.as_mask(n_cases))
