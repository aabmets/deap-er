...
def _mate_pair(toolbox: Toolbox, population: list[GPIndividual]) -> tuple[Any, Any]:
...
def _breed_into(
    ...
    if op_random < cx_prob:
        child1, child2 = _mate_pair(toolbox, population)
        _append_if_accepted(produced_pop, produced_pop_sizes, child1, accept_func)
        if len(produced_pop) < count:
            _append_if_accepted(produced_pop, produced_pop_sizes, child2, accept_func)
        return
    child = _mutate_or_clone(toolbox, population, op_random, cx_prob, mut_prob)
    _append_if_accepted(produced_pop, produced_pop_sizes, child, accept_func)
...
def _produce(
    ...
    while len(produced_pop) < count:
        if pick_from:
            _append_if_accepted(produced_pop, produced_pop_sizes, pick_from.pop(), accept_func)
            continue
        _breed_into(
            toolbox,
            population,
