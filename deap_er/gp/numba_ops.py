...
from . import _numba_kernels, _numba_numeric, _numba_predicate, _numba_window
from .opcodes import USER_BASE, Tape
...
def _build() -> tuple[Any, Any]:
    ...
    for module, names in _JIT_GROUPS:
        for name in names:
            ...
            setattr(_numba_kernels, name, compiled)
        if module is _numba_numeric:
            _numba_predicate._truthy = _numba_numeric._truthy
    _built["run"] = jit(_numba_kernels._interpret)
    _built["idle"] = jit(_numba_kernels._idle)
    return _built["run"], _built["idle"]
